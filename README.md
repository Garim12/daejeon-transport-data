...
시간표의 `displayLabel`과 `displayCategory`는 선택적인 화면 표시 필드입니다. 기존 파일처럼 두 필드가 없어도 유효합니다. `displayCategory`는 `vacation` 또는 `special`을 사용하며, 값이 있으면 `displayLabel`도 있어야 합니다. 이 표시는 화면과 예약 스냅샷에만 쓰이고 출발시각·알람 ID·근무 날짜·Native 예약 계산에는 관여하지 않습니다.
